...
// We will look for className={`fixed z-[100] bg-white dark:bg-[var(--bg-card)] transition-all duration-300 ease-in-out flex flex-col overflow-hidden ...
// and replace up to rendering it.
...
const copilotHeaderReplaceStr = `  {/* Header */}
  ...
  {!isInline && (
    <button
      ...
      className="p-1.5 hover:bg-[var(--bg-card-hover)] dark:hover:bg-zinc-800 rounded-lg text-neutral-550 dark:text-[var(--text-primary)] cursor-pointer mr-1 active:scale-95 transition-all"
      title="Go Back"
    >
...
// We will look for the exit close button block precisely using index-of
const exitButtonIndex = appContent.indexOf('{/* Exit/Close Chat button */}');
if (exitButtonIndex !== -1) {
  // Let's find the closing tag of button and the closing code sequence
